...
import ServiceModalSaleskeywordRankKeywordHanok from "../services/ServiceModalSaleskeywordRankKeywordHanok";
...
export function ChildKeyword({ index, data, onDelete }) {
  ...
  const fnSubmitKeyword = (e) => {
    ...
    if (childInput.hskid !== "") {
      // 수정
      API.servicesPostData(APIURL.urlSetHanokSalesKeyword, {
        hskid: childInput.hskid,
      ...
      // 추가
      API.servicesPostData(APIURL.urlAddHanokSalesKeyword, {
  ...
  const confirmDelete = () => {
    if (window.confirm("정말로 이 키워드를 삭제하시겠습니까?")) {
      API.servicesPostData(APIURL.urlSetHanokSalesKeyword, {
        ...
        console.log("Deletion response:", childInput.hskid);
        TOA.servicesUseToast("키워드가 삭제되었습니다.", "s");
        RAN.serviesAfter2secondReload();
      });
      // .catch((error) => {
      //   TOA.servicesUseToast("삭제 중 오류가 발생했습니다.", "e");
      // });
    }
  };
  ...
  return (
    <>
      ...
      {rankClick && (
        <ServiceModalSaleskeywordRankKeywordHanok
          click={rankClick}
      ...
      <ul>
        ...
        <li style={{ width: "66%" }}>
          ...
          <input
            ...
            disabled={!!childInput.hskid ? true : false}
          />
          <button
            type="button"
        ...
        <li style={{ width: "10%" }}>
          ...
          <button
            ...
            style={{ backgroundColor: "#9b111e", color: "#fff" }}
          >
            삭제
          </button>
          {/* <button
...
export default function ComponentCompanySealsKeywordHanok() {
  const [keywordInput, setKeywordInput] = useState([]);
  const { cid } = useParams();
  const [clickModal, setClickModal] = useState(false);

  useEffect(() => {
    API.servicesPostData(APIURL.urlListHanokAdminSalesKeyword, {
      rcid: cid,
    }).then((res) => {
